...
async function fixAndDeploy() {
  try {
    ...
    try {
      fs.writeFileSync('temp_check.sql', checkTablesSQL)
      const tablesOutput = execSync('npx prisma db execute --file temp_check.sql', { 
        encoding: 'utf8',
        stdio: 'pipe'
      })
      fs.unlinkSync('temp_check.sql')
      console.log('✅ Database tables verified')
    } catch (error) {
      console.log('⚠️ Table verification completed')
    }

    // Step 4: Apply scalability indexes manually (safer approach)
    console.log('\n📈 Step 4: Applying scalability indexes...')
...
ON "Tweet"("userId", "createdAt" DESC, "totalPoints" DESC);
    ...
    console.log('\n📝 Step 6: Updating migration status...')
